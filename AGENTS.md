...
이 저장소에서 작업하는 모든 AI 에이전트는 작업 전에 다음을 순서대로 확인한다.

1. `CLAUDE.md` — 장기적으로 유지할 브랜드·카피·개발 원칙
2. `HANDOFF.md`의 맨 위 **`지금 상태`** 블록 — 채널 지도, 지금 손대지 않을 것, 고정값. **채널이나 외부 계정을 건드리기 전에 반드시 본다.**
3. `HANDOFF.md`의 날짜 항목 — 경위 기록이다. 151개가 쌓여 있으므로 **전부 읽지 말고 다루는 주제만 찾아 읽는다.**
4. `COLLABORATOR_BRIEF.md` — 공동 작업자용 시작·배포·복구 요약
