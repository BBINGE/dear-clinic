...
> 2026-09-06 인수인계: 이 문서는 공개 전 검증의 이력이다. 현재는 공개 ON이며 기본 미체크 + 사용자가 누르는 전체 동의, 소형 동의 카드, 증상 응대 보정, 네 언어 방침 정리까지 배포했다. 현재 작업은 [HANDOFF.md](../HANDOFF.md)의 최상단 운영 요약을 따른다. 아래 OFF·전체 동의 없음·공개 예정 표현을 현재 상태나 재실행 지시로 사용하지 않는다. 공개 모드 재검사는 `node tools/smoke-dear-ai-release.mjs --public`이며 유료 API를 호출한다. 공급자 문의를 승인 없이 발송하거나 회신을 임의의 필수 관문으로 만들지 않는다. 미확정 사실이 해소됐다는 뜻은 아니다.
