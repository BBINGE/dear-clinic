...
try {
  ...
  assert.match(index, /class="column-featured js-reveal" href="columns\/chemotherapy-pain-clinic\.html" data-journal-number="26"/);
  ...
  assert.equal(collectionSchemaText.mainEntity.numberOfItems, 26);
  assert.equal(collectionSchemaText.mainEntity.itemListElement[0].url, "https://dearhani.com/columns/chemotherapy-pain-clinic.html");
  ...
  assert.match(rss, /<lastBuildDate>Thu, 03 Sep 2026 15:00:00 GMT<\/lastBuildDate>/);
  assert.equal(
  ...
  assert.equal(latestColumnMenu.slug, "chemotherapy-pain-clinic");
  assert.equal(latestColumnMenu.href, "/columns/chemotherapy-pain-clinic.html");
  assert.equal(latestColumnMenu.image, "/assets/images/columns/chemotherapy-pain-cover-disungi-v1.png");
  ...
  assert.match(fs.readFileSync(path.join(testRoot, "columns.html"), "utf8"), /class="column-featured js-reveal" href="columns\/chemotherapy-pain-clinic\.html" data-journal-number="25"/);
  assert.equal(JSON.parse(fs.readFileSync(path.join(testRoot, "assets", "data", "latest-column.json"), "utf8")).slug, "chemotherapy-pain-clinic");
