...
try {
  ...
  assert.match(index, /class="column-featured js-reveal" href="columns\/diet-without-hunger\.html" data-journal-number="27"/);
  ...
  assert.equal(collectionSchemaText.mainEntity.numberOfItems, 27);
  assert.equal(collectionSchemaText.mainEntity.itemListElement[0].url, "https://dearhani.com/columns/diet-without-hunger.html");
  ...
  assert.match(rss, /<lastBuildDate>Fri, 04 Sep 2026 00:00:00 GMT<\/lastBuildDate>/);
  assert.equal(
  ...
  assert.equal(latestColumnMenu.slug, "diet-without-hunger");
  assert.equal(latestColumnMenu.href, "/columns/diet-without-hunger.html");
  assert.equal(latestColumnMenu.image, "/assets/images/columns/diet-without-hunger/cover.png");
  ...
  assert.match(fs.readFileSync(path.join(testRoot, "columns.html"), "utf8"), /class="column-featured js-reveal" href="columns\/diet-without-hunger\.html" data-journal-number="26"/);
  assert.equal(JSON.parse(fs.readFileSync(path.join(testRoot, "assets", "data", "latest-column.json"), "utf8")).slug, "diet-without-hunger");
