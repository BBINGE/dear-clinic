...
const sharedCssVersion = "20260922-1";
const sharedMainVersion = "20260922-4";
...
// 부른다. 실제로 그렇게 어긋난 채 방치된 적이 있어 여기서 함께 검사한다.
{
  ...
  assert.ok(publisher.includes(`style.css?v=${sharedCssVersion}`), "칼럼 발행기의 style.css 캐시 버전이 사이트와 다릅니다");
}
...
for (const pageUrl of sitemapUrls) {
  ...
  if (relativePath === "international-appointment.html") {
    assert.match(html, /js\/international-appointment\.js\?v=20260922-4/, "외국인 예약 페이지 JS 캐시 버전이 다릅니다.");
  } else {
...
assert.match(home, /js\/weather-scene\.js\?v=20260922-2/, "날씨 장면 지연 로더의 캐시 버전이 다릅니다.");
...
assert.match(koreanPrivacy, /시행일자:? 2026년 9월 17일/, "개인정보처리방침 시행일이 갱신되지 않았습니다.");
