...
- 디숭이를 이어서 작업할 때는 [디숭이 상세 인수인계](docs/DISOONGI_HANDOFF.md)까지 읽는다. 짧은 운영 요약만으로 말투·캐릭터·개인정보 UX의 합의 배경을 추정하지 않는다.

- **2026-09-06 인수인계:** 디숭이는 일반 공개 중이며 개인정보처리방침 정리까지 배포했다. 최신 상태·합의한 말투/캐릭터/동의 UX·비용 제한·파일 지도는 [HANDOFF.md 맨 위의 `지금 상태` 블록](HANDOFF.md)부터 확인한다. 아래 과거 복구 태그를 최신 운영본으로 오인하지 않는다.
- 홈페이지 Pages와 디숭이 Cloudflare Worker는 별도 배포다. 홈페이지 push만으로 AI 서버 변경이 적용되지 않는다. 다른 PC에서도 기존 키·도메인·계정을 재생성하지 않는다.
