...
- 메인의 첫 공진단 섹션은 기존 상단 2장·하단 3장으로 나뉘었던 사진을 모두 상단 우측 프레임에 모아, 5장이 순서대로 자동 전환되는 크로스페이드 슬라이드로 통합했다. 왼쪽 확정 카피는 변경하지 않았다.
- 두 번째 공진단 섹션의 왼쪽 사진 프레임은 서울 서초의 디어한의원에서 충청·대구·부산·호남·제주로 이어지는 코드 기반 남한 지도 모션으로 교체했다. 오른쪽 확정 카피는 유지하며, 움직임 최소화 설정에서는 경로와 목적지를 정지 상태로 표시한다.
