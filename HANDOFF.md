...
- `care.html`은 기존 다섯 가지 Care 탭과 문구를 유지하면서 사진 중심 구조로 개편했다. 실제 원장 사진과 공간 사진을 참고한 문진·상태 평가·치료 준비 이미지 3장과 실제 치료실 사진을 사용한다. 진료 장면과 다섯 Care 사이에는 같은 진료 과정에서 오늘의 상태에 따라 살피는 초점이 달라진다는 연결 문장을 두었다. PC에서도 사진 하나가 화면을 독점하지 않도록 진료 장면 3장을 한 행의 비교 가능한 갤러리로 축소했고, 밝은 첫 화면의 상단 내비게이션은 초록색으로 보정했다.
...
- `js/click-tracking.js`는 전화·네이버 예약·톡톡·블로그·인스타그램·BE DEER 진입 클릭을 `dear_cta_click` 이벤트로 `dataLayer`에 기록한다. 실제 통계 수집에는 GA4 측정 ID 연결이 추가로 필요하다.
