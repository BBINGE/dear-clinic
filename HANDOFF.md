...
- 공개 지도 캡처와 주변 상호가 보이는 원본 사진은 더 이상 렌즈 배경으로 쓰지 않는다. 동일한 서초동풍 유리 건물 구도를 유지한 포토리얼 낮·밤·비 오는 밤·눈 오는 낮 배경을 각각 사용한다. 낮/밤과 강수 상태를 조합해 배경을 선택하고, 실제 사진 위에 느린 카메라 이동, 차량 장노출 빛, 광원 블룸, 렌즈 물방울, 원근 눈발을 별도 모션 레이어로 겹친다. 배경은 날씨 표현용 생성 이미지이며 실제 실시간 카메라 영상으로 안내하지 않는다.
